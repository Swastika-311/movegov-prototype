...
st.markdown('''<style>
/* LifeNav dark UI: dark surfaces + light readable text */
...
.hero h1,.hero h3,.hero p{color:#f8fafc!important}
.hero h1{font-size:3.2rem;margin-bottom:.5rem}.hero h3{font-size:1.5rem}.hero p{font-size:1.05rem;color:#d7dee8!important}
...
.stSelectbox div[data-baseweb="select"]>div{color:#f8fafc!important;background:#151a23!important;border:1px solid #475569!important}
.stSelectbox div[data-baseweb="select"] span{color:#f8fafc!important}
...
.tracker-item{padding:1rem 1.2rem;margin:.6rem 0;border:1px solid #303846;border-radius:14px;background:#151a23}
.tracker-done{border-color:#365c46;background:#13231a}
...
def ensure_user():
    ...
    except requests.RequestException as exc:st.error(f'Could not start the LifeNav journey: {exc}');st.stop()
...
def set_done(user_id, service_id, done, apps):
    ...
    if done:
        if app:
            ...
            if created:api('PATCH',f"/applications/{created['id']}",json={'status':'Done'})
    elif app:
...
if page=='Home':
    ...
    completed=sum(a['status'] in {'Completed','Done'} for a in apps);st.progress(completed/max(len(recs),1),text=f'{completed}/{len(recs)} tracked actions completed')
    ...
    if 'detail' in st.session_state:
        ...
        if a:
            statuses=['Not Started','Preparing','Submitted','Under Processing','Action Required','Completed','Done'];ns=st.selectbox('Update status',statuses,index=statuses.index(a['status']) if a['status'] in statuses else 0,key='status_'+s['service_id'])
    ...
    try:
        ...
        if not recs:recs=build_plan(u['id'])
        apps=api('GET',f"/applications/{u['id']}")
    except requests.RequestException as exc:st.error(f'Could not load your journey checklist: {exc}');st.stop()
    ...
    for i,r in enumerate(recs,1):
        ...
        st.markdown(f"<div class='tracker-item {'tracker-done' if done else ''}'><h3>{i}. {r['service_name']}</h3><p>{r['reason']}</p>{'<p class=\"done-label\">✓ Done</p>' if done else '<p class=\"muted\">Not completed yet</p>'}</div>",unsafe_allow_html=True)
        checked=st.checkbox('Mark as Done',value=done,key=f'journey_done_{r["service_id"]}')
        if checked != done:
            try:
                set_done(u['id'],r['service_id'],checked,apps)
                st.rerun()
            except requests.RequestException as exc:st.error(f'Could not update {r["service_name"]}: {exc}')
    completed=sum(1 for r in recs if next((x for x in apps if x['service_id']==r['service_id']),None) and next(x for x in apps if x['service_id']==r['service_id'])['status']=='Done')
