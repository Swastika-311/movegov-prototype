...
def seed_if_empty():
    ...
    try:
        ...
        if not db.query(User).filter_by(name='Demo Citizen').first():
            u=User(name='Demo Citizen',current_city='Prayagraj',destination_city='Lucknow',state='Uttar Pradesh',move_date=date.today(),move_type='Permanent',reason='Employment')
