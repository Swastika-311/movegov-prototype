...
def answer(query, user_context, recommendations, applications):
    ...
    if 'first' in q or 'start' in q:
        ...
        text='Start with the high-priority actions shown in your journey, especially those that are directly applicable to your profile. MoveGov does not submit applications for you.'
    elif 'document' in q:
        text='For the relevant services, use the documents listed in each service card. Where an exact document list is state- or workflow-specific, MoveGov marks it for verification rather than guessing.'
    else:
